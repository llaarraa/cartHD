...
#' \item{class.train}{true class membership of the training set samples}
...
#' \item{folds.cv}{samples left out from each cross-validation fold}
...
########## add an option for loo-cv, in this case it does not use balaced folds to produce the folds, but makes the complete list
		...
		if(nfolds<num.samples) {my.folds=.balanced.folds(y, nfolds) 
								my.folds=lapply(my.folds, sort)} else {my.folds=as.list(1:num.samples)
																		#if the number of specified folds is larger than the number of samples perform leave-one-out cross-validation
																		nfolds=num.samples
																		}
								
		#if the CV progress must be printed on screen, an index is initialized
			if(verbose==TRUE) {cat("Performing cross-validation using", nfolds , "folds \n")
								my.env=environment()
								#.internal.index=1						
								.internal.index=1						
									}
		...
		#26/4/2013: added the output of CV score, basides class membership. the score is the evaluated probability of being in class 1 for the left out samples. the probability is derived from fitted CART ($p1Node.r or $p1Node.l)
		predictions.scores.cv = lapply(my.folds, .cart.for.cv.internal.with.arguments, N, my.ranks, num.var, my.tr.s, my.y.s, my.w.s, my.y.0, my.data.train,  my.weights, y, min.samples, max.depth, min.gain, verbose, my.env)
		
		
		prediction.cv=(unlist(predictions.scores.cv)[seq(1, N*2, by=2)])[order(unlist(my.folds))]
		score.cv=(unlist(predictions.scores.cv)[seq(2, N*2, by=2)])[order(unlist(my.folds))]
...
return(list(results.cart=Results.cart, prediction.cv=prediction.cv, class.train=y, score.cv.class1=score.cv, accuracy.cv.all=mean(prediction.cv==y), accuracy.cv.class1=mean(prediction.cv[y==1]==y[y==1]), accuracy.cv.class0=mean(prediction.cv[y==0]==y[y==0]), folds.cv=my.folds ))
