#' CART for binary outcomes using easy ensemble with cross-validated prediction, evaluates the predictive accuracy of the model with cross-validation 
#'
#' Fit of CART using easy ensemble - a method that uses downsized class balanced training sets and boosting (with internal cross-validation) - with class cross-validated prediction for training data.  
...
#' @param nfolds.Icv number of folds to be used for internal cross-validation (within boosting for the evaluation of the error)
...
function(my.data.train, y, my.weights=rep(1/nrow(my.data.train), nrow(my.data.train)), my.prior=NULL, min.samples=3, min.gain=0.01,  
			max.depth=5, 
			nfolds=5, 
			num.boost=10, nfolds.Icv=5, num.ee=10, replace.ee=FALSE, check.data=TRUE, verbose=TRUE){
...
################## step 0: calculate some quantities needed to fit cart (multiple times)
			...
			############################# step 2: obtain predictions on the training set (cross-validated!) ############################
			#prediction.cv=my.predict.cart(results.cart, my.data.train) - not cross-validated, used in the Icv version of the function
			#### not necessary?? 
			########################## ############################################################## ##########################		
	...
	#updated 8/5/2013: new function for Icv+cv
	y.pred=lapply(my.folds.cv, .cart.for.cv.ee.Icv.internal.with.arguments, N, my.ranks, num.var, my.tr.s, my.y.s, my.w.s, my.y.0, my.data.train,  my.weights, y, min.samples, max.depth, min.gain, num.ee, num.boost, nfolds.Icv, verbose, my.env)
